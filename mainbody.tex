 ...
 First,
  unlike previous works only monitoring the double and add functions of the scalar multiplication in the cache side channel attacks,
   we add an extra monitor on the invert function
    and get a Double-Add-Invert chain through the Flush+Flush side channel.
   %and
...
To the best of our knowledge, this work is the first one to obtain and exploit \emph{\textbf{all signs}} of the wNAF representation
    in the side channel attacks against ECDSA,
     and requires the least number of signatures for the ECDSA private key recovery.
