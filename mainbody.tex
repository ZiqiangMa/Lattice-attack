...
Compared with the existing works,
 ...
 First, 
  Instead of just monitoring the double and add functions of the scalar multiplication in the cache side channel attack, 
   we add a extra monitor to the invert function 
...
From this chain, 
    $153.2$ bits information of the ephemeral key will be extracted per signature
    for 256-bit ECDSA,
    much more than existing methods.
Then, to efficiently use the extracted information, 
...
We applied our attacks to ECDSA with the secp256k1 curve in OpenSSL 1.1.1b,
  and implemented the Flush+Flush method to monitor the functions of double, add and invert.
