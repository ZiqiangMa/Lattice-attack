...
\begin{itemize}
  ...
  \item
    ...
    Depending on the reloading time, the attacker determines whether the memory lines are accessed by the victim.
    If the time is longer, it means that the attacker reloads the data from the memory.
...
The Flush+Flush attack \cite{gruss2016flush} also needs the shared memory.
...
The HNP problem can be converted to the CVP/SVP problem in the lattice and solved by the lattice basis reduction algorithm.
