...
In OpenSSL, scalar multiplication in the prime field is implemented using the wNAF representation \cite{GORDON1998129,Miyaji1997,Koyama1002,Solinas2000} of the scalar $k$,  between versions  0.9.7 and 1.1.0h.
...
Also, OpenSSL uses the modified wNAF representation instead of the generalized one stated in Algorithm~\ref{alg:wnaf}, to avoid length expansion in some cases and achieve more efficient exponentiation.
...
In OpenSSL versions between 0.9.7 and 1.1.0h~\cite{openssl}, the processes of scalar multiplication with wNAF representation are the same,
 while  the processes are encapsulated in different functions for different versions.
Here, we adopt OpenSSL 1.1.0h as  the example to describe the process.
...
In this section, we introduce the side channel attacks on the symmetric and asymmetric cryptography,
 then describe the extension of these attacks in cloud environment,
  and finally provide the details of the Flush+Reload \cite{flushreload} and Flush+Flush \cite{gruss2016flush} which are widely in the attacks on the ECDSA.
...
The execution time of \verb+clflush+ is less than the reload time on average,
    and the first and  third phase are merged together in the Flush+Flush attack.
