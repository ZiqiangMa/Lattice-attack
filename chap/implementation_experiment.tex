...
Similar to \cite{Fan2016}, we first translate the problem of recovering ECDSA secret key to the EHNP Problem. Then we further translate it to the problem of solving approximate SVP Problem using lattice reduction algorithm.
     ...
     Then, we guess some $k_{i,j}$ to reduce the dimension of lattice and improve the success probability. Finally, we observe that BKZ will recover different samples with different $\delta$'s, therefore, we provide some plausible reasonable $\delta$'s and use them to improve success probability.
     ...
     We applied our attack to the secp256K1 curve. 3 signatures is enough to recover the secret key with a high success probablity not less than $73\%$. When we have only two signatures, we can recover the secret key with a success probability no less than $24.3\%$.
       ...
       \subsubsection {Reduction to EHNP Problem}
