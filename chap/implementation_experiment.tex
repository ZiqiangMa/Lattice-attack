...
In this section, we apply our method introduced in Section \ref{sec:attack} to attack the \textbf{secp256k1} curve in OpenSSL 1.1.1b.
...
For the experiments, we use the Flush+Flush to attack the 256-bit curve \textbf{secp256k1}.
...
Also we can determine the start position by monitor the code of the copy function in OpenSSL.
...
Fig.~\ref{fig1} shows a fragment of the output of the spy when OpenSSL performs ECDSA with the \textbf{secp256k1} curve.
...
We apply our lattice attack to the curve \textbf{secp256k1} in our experiments, and we assume that the Flush+Flush attack is perfect, that is we can correctly obtain the ``double-add-invert" chain and recover all the information about the digits of the ephemeral key it contains.
...
It is able to extract $47.6$ bits per signature on average for the \textbf{secp256k1} curve and recover the private key with 13 signatures.
...
They managed to obtain on average 105.8 bits per signature for the \textbf{secp256k1} curve
