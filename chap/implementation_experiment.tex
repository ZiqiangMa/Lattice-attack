...
After we obtain the data from the cache side channel, we will use this data to recover the ECDSA private key.
 The most intuitive idea is that first recovering some bits of the ephemeral key $k$ from the Double-Add-Invert chain and
  then constructing the HNP or EHNP with these bits.
% So we first try to recover the bits of the ephemeral key and construct the HNP problem to recover the ECDSA private key.
So we first recover the consecutive bits at the position of every non-zero digits for the ephemeral key $k$.
 Then we construct the HNP problem using these bits and solve it by the approximate CVP/SVP Problem with the lattice reduction algorithm to recover the ECDSA private key.
 
%Another method is like Fan~\cite{Fan2016} and Van de Pol~\cite{Van2015}, directly exploiting the wNAF representation to construct the  HNP or EHNP with the known information from the Double-Add-Invert chain.
%Then we directly construct the EHNP problem  with the wNAF representation referring to Fan's method in the next section.

%In this section, we first recover the consecutive bits at the position of every non-zero digits for the ephemeral key $k$, exploiting the information obtained from the side channel.
...
We applied the attack to the secp256k1 curve.
...
First, we denote the wNAF representation of $k$ as $k = \sum{k_{i}2^{i}}$,
 ...
 But for arbitrary non-zero digits, it can not directly determine whether the bit corresponding to the position of the digit is zero or one.
...
\label{data_proc2}
In this section, we transform the problem of recovering the private key to the HNP instance, and further convert it to the CVP/SVP instance in a lattice.
Our method is based on the analysis from \cite{Nguyen2002}.
And we make the following improvements to it.
...
Although in Section \ref{data_proc2} the HNP problem introduced requires that $l$ should be larger than $7$.
...
\begin{table}[t]
  ...
  \label{svp9}
    \begin{tabular}{|c|m{0.3in}<{\centering}|m{0.3in}<{\centering}|m{0.3in}<{\centering}|m{0.3in}<{\centering}|m{0.3in}<{\centering}|m{0.3in}<{\centering}|}
    \hline
    \multirow{3}{*}{Dimension}&
    \multicolumn{6}{c|}{Block size}   \\ %
    \cline{2-7}
    ~& \multicolumn{2}{c|}{$l \geq 9$} & \multicolumn{2}{c|}{$l \geq 10$} & \multicolumn{2}{c|}{$l \geq 11$}  \\
    \cline{2-7}
    ~& 10 & 20 & 10 & 20  & 10 & 20\\
    %\hline
  \hline
  ...
  \hline
  90 & 0.5  & 1.0 & 14.5 & 22.5 & 67.0 & 74.0 \\
  \hline
  100 & 0.5  & 4.0 & 21.0 & 33.5 & 70.5 & 83.5\\
  \hline
  110 & 1.5  & 3.0 & 16.0 & 36.5 &  79.5 & 86.5\\
  \hline
  120 & 2.0  & 4.5 & 21.5 & 35.0 &  77.5  &92.0\\
  \hline
  130 & 0.5  & 2.5 & 24.0 & 45.5 &  83.0  &94.5\\
  \hline
  140 &  3.0 & 8.0 & 29.0 & 46.0 &  88.5   &99.0\\
  \hline
  150 & 3.0  & 6.0 & 32.0 & 49.0 &  88.0  &99.0\\
  \hline
  160 & 2.5  & 13.0 & 27.5 & 49.0 &  94.0  &99.5\\
  \hline
  170 & 5.0  & 12.5 & 39.0 & 56.5 &  93.0  &99.5\\
  \hline
  180 & 3.5  & 11.5 & 37.0 & 57.0 &  97.0  &100.0\\
  \hline
  190 & 7.0  & 19.5 & 39.0 & 63.5 &  98.0  &100.0\\
  \hline
  200 & 9.5  & 26.0 & 46.0 & 66.0 &  99.0  &100.0\\
  \hline
  210 & 10.5  & 22.0 & 51.5 & 66.0 & 99.5  &100.0 \\
  \hline
  220 & 15.0  & 29.0 & 51.0 & 72.5 & 100.0  & 100.0 \\
  \hline
  230 & 19.0  & 30.0 & 58.5 & 73.5 & 99.0 & 100.0\\
  \hline
    \end{tabular}
\end{table}
...
We can successfully recover the private key of a 256-bit ECDSA only need $60$ sequences of consecutive bits with a success probability of $7.5\%$.
These $60$ sequences come from up to $60$ signatures.
That means we just need about $60$ signatures satisfied the length requirement to successfully recover the ECDSA private key.

We analysed 10000 signatures.
$32.33\%$ signatures contain the sequence of consecutive bits that $l \geq 9$,
$17.44\%$ signatures contain the sequence that $l \geq 10$
 and $9.08\%$ signatures contain the sequence that $l \geq 11$.
 So, obtaining 60 satisfying signatures ($l \geq 11$) needs 661 signatures totally.
However, when using 80 signatures that satisfy $l \geq 9$, the total number of signatures needed is the least, just $248$.

 %因此60 个满足条件的签名需要的总签名数量为661。而当使用80 个
%满足条件的签名，要求连续比特长度大于等于9 时，攻击需要的总签名数最少，
%为247.45。
...
Also, the higher block sizes perform with a higher success probability,
 as the stronger reduction allows them to isolate the solution vector better.
  We believe that the success probability would be  higher with the block size $30$ or the BKZ 2.0~\cite{bkz2}.
...
We can see that the restriction on the length of the bits has a great influence on the number of signatures we need to observe.
Although we have exploited the sign information of the wNAF representation, the result is not as expected, worse than Allan's attack~\cite{Allan2016}, which only needs 6 signatures.
So, in this section, we directly exploiting the wNAF representation to construct EHNP problem with the known information from the Double-Add-Invert chain
    to recover the ECDSA private key.
The final result shows that this method only needs 2 signatures, better than existing results.
...
\subsection{ Extracting more Information }
 ...
 k&=&\sum\limits_{i=1}^{l} (-1)^{h_i} k^{*}_i 2^{\lambda_i} = \sum\limits_{i=1}^{l} (-1)^{h_i}(2k_i+1)2^{\lambda_i}  \\
...
Of course, this method will enlarge the time complexity, however, comparing with the improvement of success probability, the increment of time complexity is acceptable. The related data basing on the ways of balancing, guessing and different $\delta$s can be found in Table \ref{table3}. We can see the probability will increase to $24.3\%$ when using 3 different $\delta$s, while the time complexity will approximately increase by 2 times. Compared with the experiment using guessing 4 $k_{i,j}$s in Table \ref{table2}, the way of combining guessing 2 $k_{i,j}$s and different $\delta$s is more efficient.
...
\begin{tabular}{|c|m{90pt}|c|c|c|}
  ...
  \hline
  Allan et al.~\cite{Allan2016} & half positions and signs of non-zero digits & HNP & 71.4 & 6 \\
  \hline
  ...
  \hline
  \multirow{2}{1.2in}{\centering Ours}&\multirow{2}{90pt}{all positions and signs of non-zero digits} & HNP & 154.2  & 248 \\ %
    \cline{3-5}
  ~& ~& EHNP & 154.2  & 2  \\
%  \hline
%  190 & 39.3 & 63.5 & & \\
...
Allan et al.~\cite{Allan2016} first used the Flush+Reload attack to monitor the invert function and used a performance degradation attack to increase the attack accuracy.
 Then they exploited Van de Pol's method to construct the HNP problem with the extra information to recover the ECDSA private key.
  This method can extract $71.4$ bits per signature on average for the secp256k1 curve and recover the private with 6 signatures.
...
We compare these attacks with ours in four aspects as shown in Table \ref{compare1}.
...
We  use both the HNP and EHNP problems to recover the ECDSA private key.
Although the using HNP problem needs 248 signatures,
using EHNP problem the number of signatures needed to recover the private key is only $2$ with the success probability being not less than $24.3\%$.
