...
In practical, only when $l > 8$, we successfully recover the ECDSA private key in our experiments.
...
Although in theory we only need $2$ signatures to recover the 256-bit private key,
 in practical we need at least $60$ signatures to get enough information satisfied the restriction on $l$ to construct the lattice attack.
