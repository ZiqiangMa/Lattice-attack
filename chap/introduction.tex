...
Therefore, we add a monitor to the invert function so that
 we achieve more information about the ephemeral key compared to previous attacks.
After that, we want to use all the information achieved to recover the ECDSA private key.
we manage to recover the consecutive bits at the position of every non-zero digits of the ephemeral key based on the information obtained from the cache side channels.
Then, we construct an EHNP \cite{ehnp} instance making use of all the consecutive bits of the ephemeral key
to recover the ECDSA private key by converting it to the SVP problem and solving it with lattice reduction algorithms.
...
This result is the best one as ever known.
  ...
  First, we
   ...
   Second, 
    We extract multiple consecutive bits of the ephemeral key by converting the wNAF representation to the binary representation and 
     construct the EHNP instance to recover the ECDSA private key.
  \item
