...
This helps us to determine the sign of the non-zero digits of the ephemeral key.
 % by exploiting the \emph{$EC\_POINT\_invert()$} function we can improve the cache side channel attacks.
  We get the sign of the non-zero digits in the wNAF of the ephemeral key through adding a monitor to the invert function.
...
We apply this method to attack the secp256k1 curve in this paper.
We choose the Flush+Flush\cite{gruss2016flush} method to implement the cache side attack due to its higher precision.
...
Compared to the previous works, ours obtains more information about the ephemeral key, i.e. the sign of all non-zero digits, from the side channels.
...
The private key is retrieved by solving the HNP instance using lattice reduction algorithm.
  ...
  \item
   ...
   The experiments show that  154.2 bits are obtained on average per signature through from the cache side channel.
    Also, $60$ signatures are enough to recover the private key with the probability being about $1\%$.
\end{itemize}
