...
Then, they extract and exploit the different information of the ephemeral key from this chain to construct an instance of HNP or EHNP for the lattice attack.
...
Van de Pol et al.~\cite{Van2015}  exploited the positions of higher half non-zero digits of the ephemeral key's wNAF representations.
Fan et al.~\cite{Fan2016} constructed the EHNP to recover the private key of ECDSA and reduced the number of required signatures to $4$.
%The number of required signatures  is reduced to $4$.
Wang et al.~\cite{Wang2017} exploited the positions of two non-zero digits together with the length of the ephemeral key's wNAF representation to construct an HNP instance, which needs $85$ signatures. % to recover the private key.
%这一部分仍需要修改
