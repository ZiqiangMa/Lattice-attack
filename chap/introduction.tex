...
Due to the small key size and high security, it is widely used in many popular applications, such as TLS protocol \cite{rfc5246}, OpenPGP \cite{openpgp2007}, smart card \cite{smartcard2007}, and Bitcoin \cite{bitcoin2008}.
...
The scalar multiplication of a point over elliptic curves by a random ephemeral key is the core operation of ECDSA.
The private key can be extracted if the ephemeral key (or implementations) is revealed,
so this ephemeral key must be kept secret.
...
Then, Brumley and his colleagues employ this lattice attack to recover ECDSA keys from leaked LSBs (in \cite{Brumley2009}) and leaked MSBs (in \cite{Brumley2011}).
...
In 2015, Van de Pol et al. improved this method by deriving a more effective way of exacting information from the cache side channel \cite{Van2015},
 which used the information about the position of higher half non-zero digits of the wNAF representation of the ephemeral key.
...
In 2016, Goudarzi et al.~\cite{Dahmun2016}  extended the basic lattice attack to the ephemeral keys,  which are blinded by the addition of a random multiple of the elliptic-curve group order or by a random Euclidean splitting.
While in \cite{Fan2016}, Fan et al. proposed a new way of extracting and utilizing information from the Flush+Reload attack. The problem of recovering the
...
In 2017, Wang et al.~\cite{Wang2017} gave a lattice attack on ECDSA using
...
To some extent,
they just attempted to make better use of the data achieved through the side channels and construct more effective lattice attacks
...
In terms of this respect, though the methods are different in these works, no effort and improvement have been made to achieve more information.

In our paper, we also focus on the ECDSA algorithm with the wNAF representation of the ephemeral key, especially the implementation in the OpenSSL library.
...
We expect not only to make better use of the information,
  but also to extract more information from the side channels.
...
It may help us to determine the sign of the non-zero digits of the ephemeral key.
 % by exploiting the \emph{$EC\_POINT\_invert()$} function we can improve the cache side channel attacks.
  Based on this fact, we obtain the sign of the non-zero digits in the wNAF of the ephemeral key by adding a monitor to the invert function.
...
This method is applied to attack the \textbf{secp256k1} curve in OpenSSL1.1.1b in this paper.
We choose the Flush+Flush~\cite{gruss2016flush} attack as the implementation of the cache side channel attack.
...
The results show that we effectively recover the ECDSA private key.
If the result of the Flush+Flush attack is perfect, we need $60$ signatures at least to recover the private key with a probability of $3\%$.
...
Compared to previous works, our proposal can obtain more information about the ephemeral key, i.e. the sign of all non-zero digits.
...
The private key is recovered by solving the HNP instance using lattice reduction algorithm.
  ...
  \item
    We apply our method to attack the \textbf{secp256k1} curve in the latest version of OpenSSL.
