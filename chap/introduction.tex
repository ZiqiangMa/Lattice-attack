...
The core operation of ECDSA sign/verify is the scalar multiplication of a base point (or generator) over elliptic curves
 ...
 And the security of ECDSA relies on the intractability of the elliptic curve discrete logarithm problem (ECDLP), that is
...
The basic idea is to construct a Hidden Number Problem (HNP) with the partial knowledge of the ephemeral keys,
 and then reduce it to the Closest Vector Problem (CVP) or the Shortest Vector Problem (SVP) in a lattice to calculate the private key.
...
The difference of these works is that different parts of the the ephemeral key (i.e., the corresponding wNAF representation) are used to construct the lattice attacks.
...
While, no other side channel is constructed to obtain extra information about the ephemeral key.
%以下为原来的内容
