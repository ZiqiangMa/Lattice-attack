...
With the cache side channel attacks~\cite{flushreload, liu2015last, gruss2016flush}, various practical attacks~\cite{Benger2014,Van2015,Fan2016,Wang2017} are proposed to attack  the ECDSA implementation of OpenSSL~\cite{openssl}, in which the windowed Non-Adjacent Form (wNAF) implementation is adopted as the default one for scalar multiplication between  version 0.9.7 and 1.1.0h.
...
This method~\cite{Fan2016} extracts all positions of digits and obtains $105.8$ bits per signature on average for the 256-bit elliptic curve, which performs remarkably better than other attacks~\cite{Benger2014,Van2015,Wang2017}.
...
For example,  existing methods ~\cite{Fan2016,Benger2014,Van2015,Wang2017} require $4$, $200$, $13$ and $85$ signatures, respectively, to recover the private key.
%这一部分仍需要修改
...
First, we find that the invert function is another exploitable vulnerability~\cite{Allan2016} in the implementation of the scalar multiplication in OpenSSL. We monitor the invert function in addition to points addition and doubling functions in the cache side channel, and then construct a Double-Add-Invert chain instead of the Double-Add chain~\cite{Benger2014,Van2015,Fan2016,Wang2017}.
...
%    We use the Flush+Flush technique to improve the cache side channel attacks proposed by Allan~\cite{Allan2016} to get higher resolution without using the performance degradation.
  \item We present an efficient cache side channel attack to recover the private key of ECDSA with the wNAF representation. First, we construct a Double-Add-Invert chain instead of the commonly used Double-Add chain by monitoring the invert function. Then,
