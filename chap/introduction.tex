...
With the Flush+Reload cache side channels \cite{flushreload}, various practical attacks~\cite{Benger2014,Van2015,Fan2016,Wang2017} are proposed to attack  the ECDSA implementation of OpenSSL~\cite{openssl}, in which the windowed Non-Adjacent Form (wNAF) implementation is adopted as the default one for scalar multiplication between  version 0.9.7 and 1.1.0h.
...
After analyzing the implementation code of ECDSA in OpenSSL, we find that the invert function is another exploitable vulnerability~\cite{Allan2016} in the implementation of the scalar multiplication. We monitor the invert function in addition to points addition and doubling functions in the cache side channel, and then construct a Double-Add-Invert chain instead of the Double-Add chain~\cite{Benger2014,Van2015,Fan2016,Wang2017}.
...
We apply our attack methods to the secp256k1 curve in OpenSSL 1.1.0h.
...
While using the EHNP, $3$ signatures would be enough to recover the 256-bit ECDSA private key with the success  probability no less than $73\%$,
...
%    We use the Flush+Flush technique to improve the cache side channel attacks proposed by Allan~\cite{Allan2016} to get higher resolution without using the performance degradation.
  \item We present a new lattice attack to recover the private key of ECDSA with the wNAF representation. First, we construct a Double-Add-Invert chain instead of the commonly used Double-Add chain by monitoring the invert function. Then,
  ...
  The attack based on EHNP works better than all known attacks.
