...
Existing attacks~\cite{Benger2014,Van2015,Fan2016,Wang2017} obtain the information of the ephemeral key's wNAF representation \emph{only} from the Double-Add chain,
the number of bits extracted from each signature reaches a bottleneck. 
...
Naturally, more bits are obtained, less number of signatures is required for recovering the ECDSA private key.
For example,  existing methods ~\cite{Fan2016,Benger2014,Van2015,Fan2016,Wang2017} requires $4$, $200$, $13$ and $85$ signatures respectively. 
%这一部分仍需要修改
...
In this paper, we propose a new attack against the wNAF implementation of ECDSA. 
We obtains more information from the side channel instead of only the Double-Add chain,
 which is then fully extracted and utilized by using the EHNP and HNP.
Therefore, it obtain  $153.2$ bits per signature for 256-bit ECDSA on average,  remarkably more than the  best known result (i.e., $105.8$ bits in~\cite{Fan2016}); and require only $2$ signatures to recover the ECDSA private key.



We obtain  $153.2$ bits per signature for 256-bit ECDSA on average, remarkably  more than the  best known result (i.e., $105.8$ bits in~\cite{Fan2016}); and require only $2$ signatures to recover the ECDSA private key.
To achieve this, we not only obtain more information from the side channel than the methods based on the Double-Add chain~\cite{Benger2014,Van2015,Fan2016,Wang2017}, 
 but also fully utilize the extracted information by using the EHNP and HNP.
First, to obtain more information about the ephemeral key's wNAF representation, we construct a Double-Add-Invert chain instead of the Double-Add chain~\cite{Benger2014,Van2015,Fan2016,Wang2017}.
We monitor the invert function in addition to points addition and doubling functions in the cache side channel.
The analyse on the ECDSA implementation in OpenSSL, demonstrates that the invert function is invoked only when the sign of current non-zero digit of the ephemeral key's wNAF representation is opposite to the previous one in the scalar multiplication.
Therefore, in addition to the positions of all digits in the ephemeral key's wNAF representation, we can extract the sign of the non-zero digits by monitoring the invert function.
Secondly, to recover ECDSA private key, we translate it to the HNP and EHNP respectively.
In HNP, 
