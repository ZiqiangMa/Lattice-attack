...
Existing attacks~\cite{Benger2014,Van2015,Fan2016,Wang2017} obtain the information of the ephemeral key's wNAF representation \emph{only} from the Double-Add chain, from which different numbers of bits are extracted and exploited to construct the lattice attacks for the private key  recovery.
...
First, we monitor the invert function in addition to points addition and doubling functions in the cache side channel, and construct a Double-Add-Invert chain instead of the Double-Add chain~\cite{Benger2014,Van2015,Fan2016,Wang2017}.
...
Therefore, in addition to the positions of all digits in the ephemeral key's wNAF representation, we can extract the signs of the non-zero digits by monitoring the invert function.
...
However, as analyzed in Section~\ref{data_proc2}, the construction of the HNP instance requires the length of the consecutive bits larger than $7$, 
 and therefore needs more signatures, i.e., $60$.
...
We choose the Flush+Flush~\cite{gruss2016flush} attack to monitor the functions of double, add and invert, and construct the Double-Add-Invert chain.
From this chain, we successfully determine whether each digit of the ephemeral key is zero or not,
...
elliptic curve.
When using the HNP to recover the ECDSA private key, we need about $60$ signatures to recover the private key with a probability of $3\%$.
...
\begin{itemize}
  ...
  We construct the HNP and EHNP instances respectively, to utilize the positions and signs of all non-zero digits in the ephemeral key’s wNAF
