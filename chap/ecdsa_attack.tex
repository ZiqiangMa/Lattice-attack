...
We also use this vulnerability to improve the attack to obtain more valid data.
...
\subsection{Improvement Through the Flush+Flush Attack}
...
So the length of the time slot needs short enough to guarantee the attack granularity while the attack working properly.
  ...
  Also, victim access to a memory location can occur at the same time the adversary test if it is cached.
...
However, in our attack, we need to add a monitor for the invert function, so we need a larger time slot.
...
First, the cost of Flush is about 160 cycles less than the cost of Reload.
...
\label{ffattack}
We launched the Flush+Flush attack to obtain the partial information of the ECDSA on an Acer Veriton T830 running Ubuntu 16.04.
